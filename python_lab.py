...
S_intersect_T = {el for el in S if el in T}
...
first_of_tuples_list = [(x,y,z) for x in S for y in S for z in S if (x+y+z==0) and  (x,y,z) != (0,0,0) ][1]
...
range_and_zip =list( zip(range(0,5),['A','B','C','D','E','F']))
...
list_sum_zip = [sum(_tuple) for _tuple in zip(A,B)]
...
square_dict = { k:v for (k,v) in    [(x,x**2) for x in range(0,100)]  }
...
D = {'red','white','blue'}
# Replace {...} with a one-line dictionary comprehension
identity_dict = {el:el for el in D}
...
representation_dict = {x: (x // (base**2), (x % base**2 //  base) , (x % base**2 % base)) for x in range(0,1000) if x//base**2 in digits }
